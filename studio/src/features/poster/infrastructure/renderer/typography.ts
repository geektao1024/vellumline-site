...
export function drawPosterText(
  ...
  width: number,
  height: number,
  theme: Pick<ResolvedTheme, "ui" | "map">,
  center: Coordinate,
  city: string,
  country: string,
  ...
  const titleFontFamily = fontFamily
    ? `"${fontFamily}", ${POSTER_TITLE_FONT_STACK}`
    : POSTER_TITLE_FONT_STACK;
  const bodyFontFamily = POSTER_META_FONT_STACK;
  ...
  if (showPosterText) {
    ...
    ctx.font = `500 ${countryFontSize}px ${titleFontFamily}`;
  ...
  ctx.font = `400 ${attributionFontSize}px ${bodyFontFamily}`;
  ctx.fillText(
