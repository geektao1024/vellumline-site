...
declare global {
  interface Window {
    __vellumLineDeferredInstallPrompt?: Event;
  }
}
...
export default function useInstallPrompt() {
  ...
  useEffect(() => {
    ...
    setBeforeInstallPromptFired(Boolean(window.__vellumLineDeferredInstallPrompt));
    ...
    if (window.__vellumLineDeferredInstallPrompt) {
      setDeferredPrompt(
        window.__vellumLineDeferredInstallPrompt as BeforeInstallPromptEvent,
      );
    }
    ...
    const handler = (e: Event) => {
      ...
      window.__vellumLineDeferredInstallPrompt = e;
    };
    window.addEventListener("beforeinstallprompt", handler);

    const fallbackTimer = window.setTimeout(() => {
      if (isAndroid() && !window.__vellumLineDeferredInstallPrompt) {
  ...
  const diagnostics = {
    ...
    deferredPromptCached: Boolean(window.__vellumLineDeferredInstallPrompt),
