...
export default function AppShell() {
  ...
  // Desktop state
  const [desktopTab, setDesktopTab] = useState<MobileTab>("location");
  ...
  const handleDesktopTabChange = (tab: MobileTab) => {
    if (desktopPanelOpen && desktopTab === tab) {
  ...
  const handleNewDesign = useCallback(() => {
    dispatch({
      ...
      state: createInitialPosterState(),
    });
    setDesktopTab("location");
    setDesktopPanelOpen(true);
    setDesktopLocationRowVisible(true);
    setMobileTab("theme");
    setMobileDrawerOpen(false);
    setMobileLocationRowVisible(true);
  }, [dispatch]);
  ...
  return (
    <div
      ...
      data-desktop-panel={desktopPanelOpen ? "open" : "closed"}
    >
      <GeneralHeader
        onMenuToggle={() => setDesktopPanelOpen((isOpen) => !isOpen)}
      ...
      <div
        ...
        className={`desktop-left-panel${desktopPanelOpen ? " is-open" : ""}`}
      >
        <div
      ...
      <FooterNote onAboutOpen={() => setAboutOpen(true)} />
      <Suspense fallback={null}>
        <AnnouncementModal />
      </Suspense>
      {aboutOpen ? (
