...
- Share is a disclosed menu with explicit `aria-controls`; platform links stay
  secondary to Export.
- Source, license, and attribution details live in the footer, not as a top-nav
  icon. Keep the header focused on product actions.
...
- Layout options show a single ratio preview shape and the exact dimensions.
  Avoid nested SVG frames or extra preview borders inside the card.
- The layout name is centered under the ratio preview; dimensions use Spline
  Sans Mono as secondary text.
- Selected layout cards use a quiet brass hairline plus a small bottom status
  mark. Do not stack glow rings with the card border.

## Map Controls

- Desktop map controls belong to the poster, not to the page edge. Position the
  control tray below the poster and center it from the poster frame.
- The tray may be slightly wider than the poster when editing tools need room,
  but it should wrap before colliding with side panels.
- Control groups stay compact and brass-toned; do not use cyan focus glows or
  fixed right-panel anchoring.
- Rotation is a progressive control: show one brass slider, a numeric degree
  readout, and an explicit reset button. Avoid ambiguous rotate icons that look
  like reset but only nudge the map.
