...
export function Footer({ footer }: { footer: FooterType }) {
  return (
    <footer
      id={footer.id}
      className={`border-t border-border/70 py-8 sm:py-8 ${footer.className || ''} overflow-x-hidden bg-background/70 backdrop-blur-sm`}
        ...
        <div className="grid min-w-0 gap-12 md:grid-cols-5">
          <div className="min-w-0 space-y-4 break-words md:col-span-2 md:space-y-6">
            ...
            {footer.brand?.description ? (
              <p
                className="text-muted-foreground max-w-sm text-sm leading-relaxed text-balance break-words"
            ...
            <div className="flex min-w-0 flex-wrap items-center gap-4">
              {footer.agreement?.items.map((item: NavItem, index: number) => (
                <AppAwareLink
                  ...
                  href={item.url || ''}
                  target={item.target || ''}
                  className="text-muted-foreground hover:text-primary block text-xs break-words underline decoration-border underline-offset-4 duration-150"
                >
                  {item.title || ''}
                </AppAwareLink>
              ))}
            ...
            <div className="flex min-w-0 flex-wrap items-center gap-2">
              {footer.social?.items.map((item: NavItem, index) => (
                <AppAwareLink
                  ...
                  href={item.url || ''}
                  target={item.target || ''}
                  className="text-muted-foreground hover:text-primary hover:border-primary/35 hover:bg-accent/35 bg-background/70 block cursor-pointer rounded-[var(--radius-control)] border border-border/60 p-2 duration-150"
