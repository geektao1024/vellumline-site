              ...
              return (
                <NavigationMenuLink key={idx} asChild>
                  <AppAwareLink
                    ...
                    className={`text-muted-foreground flex flex-row items-center gap-1.5 rounded-[var(--radius-control)] border border-transparent px-2.5 py-1.5 text-sm whitespace-nowrap xl:gap-2 xl:px-3 ${
                      item.is_active || pathname.endsWith(item.url as string)
                        ? 'border-primary/30 bg-primary/10 text-foreground'
                        : ''
                    } hover:border-primary/25 hover:bg-accent/35 hover:text-foreground focus-visible:ring-ring/45 focus-visible:ring-2`}
                  >
      ...
      <li {...props}>
        <NavigationMenuLink asChild>
          <AppAwareLink
            href={href}
            target={target || '_self'}
            className="hover:border-primary/25 hover:bg-accent/35 grid grid-cols-[auto_1fr] gap-3.5 rounded-[var(--radius-panel)] border border-transparent p-1.5"
          >
            <div className="bg-background/70 ring-foreground/10 border-border/60 relative flex size-9 items-center justify-center rounded-[var(--radius-control)] border shadow-xs ring-1">
              {children}
            </div>
      ...
      >
        <div
          className={cn(
            ...
            'in-data-scrolled:border-border/70 in-data-scrolled:bg-background/78 in-data-scrolled:border-b in-data-scrolled:shadow-xs in-data-scrolled:backdrop-blur-xl',
            'has-data-[state=open]:ring-border/70 has-data-[state=open]:bg-card/85 has-data-[state=open]:h-[calc(var(--navigation-menu-viewport-height)+3.4rem)] has-data-[state=open]:border-b has-data-[state=open]:shadow-lg has-data-[state=open]:backdrop-blur-xl',
            'max-lg:in-data-[state=active]:bg-background/82 max-lg:h-14 max-lg:overflow-hidden max-lg:border-b max-lg:in-data-[state=active]:h-screen max-lg:in-data-[state=active]:backdrop-blur-xl'
          )}
        >
            ...
            <div className="relative flex flex-wrap items-center justify-between lg:py-5">
              <div className="flex justify-between gap-8 max-lg:h-14 max-lg:w-full max-lg:border-b">
                ...
                <button
                  ...
                  className="hover:bg-accent/35 relative z-20 -m-2.5 -mr-3 block cursor-pointer rounded-[var(--radius-control)] p-2.5 lg:hidden"
                >
                      ...
                      <AppAwareLink
                        ...
                        className={cn(
                          ...
                          button.variant === 'outline'
                            ? 'bg-background/60 border-border/70 ring-foreground/10 hover:border-primary/40 hover:bg-accent/35 dark:ring-foreground/15 border shadow-xs ring-1 duration-200'
                            : 'bg-primary text-primary-foreground border-primary/45 border shadow-sm ring-1 shadow-black/20 ring-(--ring-color) [--ring-color:color-mix(in_oklab,var(--color-foreground)12%,var(--color-primary))] hover:bg-[var(--color-accent-strong)]'
                        )}
                      >
                        {button.icon && (
                          <SmartIcon
