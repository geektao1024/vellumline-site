...
import { AppAwareLink, LazyImage } from '@/shared/blocks/common';
...
export function ShowcasesFlow({
  ...
  return (
    ...
    >
      <motion.div
        className="container mb-12 max-w-5xl text-center md:text-left"
      ...
      >
        ...
        <h2 className="mx-auto mb-6 max-w-full text-3xl font-bold tracking-normal text-pretty md:mx-0 md:max-w-5xl lg:text-4xl">
          {section.title}
        </h2>
        <p className="text-muted-foreground text-md mx-auto mb-4 line-clamp-3 max-w-full leading-relaxed md:mx-0 md:max-w-5xl">
          {section.description}
        </p>
      ...
      {groups.length > 0 && (
        <motion.div
          className="container mb-12 flex flex-wrap justify-center gap-3 md:justify-start"
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          transition={{
      ...
      {filteredItems.length > 0 ? (
        <div className="container mx-auto columns-1 gap-4 space-y-4 sm:columns-2 lg:columns-3 xl:columns-4">
          {filteredItems.map((item, index) => (
            <motion.div
              key={index}
              className="group bg-card/35 border-border/70 hover:border-primary/35 relative cursor-zoom-in break-inside-avoid overflow-hidden rounded-[var(--radius-panel)] border shadow-sm transition-[background-color,border-color,box-shadow] duration-[var(--dur-short)]"
            ...
            >
              <LazyImage
                src={item.image?.src ?? ''}
                alt={item.image?.alt ?? ''}
                className="h-auto w-full transition-opacity duration-[var(--dur-short)] group-hover:opacity-90"
                sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 25vw"
              />
              <div className="absolute inset-0 flex flex-col justify-end bg-foreground/65 p-6 opacity-0 transition-opacity duration-[var(--dur-short)] group-hover:opacity-100">
                <h3 className="mb-2 text-sm font-medium text-background">
                  {item.title}
                </h3>
                {/* {item.description && (
        ...
        {selectedIndex !== null &&
          ...
          filteredItems.length > 0 && (
            ...
            >
              ...
              <button
                className="absolute top-1/2 left-4 z-50 -translate-y-1/2 rounded-[var(--radius-control)] border border-white/15 bg-black/20 p-2 text-white/70 transition-colors hover:bg-black/40 hover:text-white"
                onClick={(e) => {
                  e.stopPropagation();
                  handlePrevious();
                }}
              ...
              <button
                className="absolute top-1/2 right-4 z-50 -translate-y-1/2 rounded-[var(--radius-control)] border border-white/15 bg-black/20 p-2 text-white/70 transition-colors hover:bg-black/40 hover:text-white"
                onClick={(e) => {
                  e.stopPropagation();
                  handleNext();
                }}
