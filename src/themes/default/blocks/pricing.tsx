...
export function Pricing({
  ...
  return (
    ...
    >
      <div className="mx-auto mb-12 max-w-3xl px-4 text-center md:px-8 md:text-left">
        {section.sr_only_title && (
          <h1 className="sr-only">{section.sr_only_title}</h1>
        )}
        <h2 className="mb-6 text-3xl font-bold tracking-normal text-pretty lg:text-4xl">
          {section.title}
        </h2>
        <p className="text-muted-foreground mx-auto mb-4 max-w-xl leading-relaxed md:mx-0 lg:max-w-none lg:text-lg">
          {section.description}
        </p>
      </div>
          ...
          {section.items?.map((item: PricingItem, idx) => {
            ...
            return (
              <Card
                key={idx}
                className={cn(
                  'relative hover:border-primary/35 hover:bg-accent/15',
                  item.is_featured && 'border-primary/45 shadow-md shadow-primary/10'
                )}
              >
                {item.label && (
                  <span className="bg-primary text-primary-foreground border-primary/35 absolute inset-x-0 -top-3 mx-auto flex h-6 w-fit items-center rounded-[var(--radius-control)] border px-3 py-1 text-xs font-medium shadow-sm shadow-primary/20">
                    {item.label}
                  </span>
                )}
                ...
                <CardHeader>
                  ...
                  <div className="my-3 flex items-baseline gap-2">
                    ...
                    {currencies.length > 1 && (
                      ...
                      >
                        <SelectTrigger
                          size="sm"
                          className="border-border/70 bg-background/70 h-6 min-w-[60px] rounded-[var(--radius-control)] px-2 text-xs"
                        >
                  ...
                  {isCurrentPlan ? (
                    ...
                    <Button
                      ...
                      disabled={isLoading}
                      className={cn(
                        'focus-visible:ring-ring inline-flex items-center justify-center gap-2 rounded-[var(--radius-control)] text-sm font-medium whitespace-nowrap transition-[background-color,border-color,color,box-shadow] duration-[var(--dur-short)] focus-visible:ring-2 focus-visible:outline-none disabled:pointer-events-none disabled:opacity-50',
                        'mt-4 h-9 w-full px-4 py-2',
                        'bg-primary text-primary-foreground hover:bg-[var(--color-accent-strong)] border border-primary/35 shadow-sm shadow-primary/20'
                      )}
                    >
                ...
                <CardContent className="space-y-4">
                      <hr className="border-border/70 border-dashed" />
