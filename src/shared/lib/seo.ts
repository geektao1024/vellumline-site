...
export function getMetadata(
  ...
  return async function generateMetadata({
    ...
    const canonicalPath = getCanonicalPath(options.canonicalUrl || '/');
    ...
    return {
      ...
      alternates: {
        ...
        languages: canonicalPath ? getLanguageAlternates(canonicalPath) : {},
      },
...
function getCanonicalPath(canonicalUrl: string) {
  ...
  return normalizeSeoPath(canonicalUrl);
}
...
async function getCanonicalUrl(canonicalUrl: string, locale: string) {
  ...
  return buildCanonicalUrl(canonicalUrl || '/', locale || defaultLocale);
}
