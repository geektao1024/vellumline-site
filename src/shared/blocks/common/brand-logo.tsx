...
export function BrandLogo({ brand }: { brand: BrandType }) {
  return (
    <Link
      ...
      className={`group flex items-center space-x-3 ${brand.className}`}
    >
      {brand.logo && (
        <Image
          src={brand.logo.src}
      ...
      {brand.title && (
        <span className="font-serif text-lg font-bold tracking-normal text-foreground group-hover:text-primary">
          {brand.title}
        </span>
      )}
    </Link>
